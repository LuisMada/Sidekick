    ...
    <div className="bg-white rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
      ...
      <div className="p-6 border-b border-gray-100">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-2">
              <h3 className="font-semibold text-gray-900">{promptBlock.name}</h3>
              <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getTypeColor()}`}>
                {getTypeIcon()}
                <span className="ml-1 capitalize">{promptBlock.type}</span>
              </span>
            </div>
            <p className="text-sm text-gray-600 line-clamp-2">{promptBlock.content}</p>
          </div>
          <div className="flex space-x-1 ml-4">
            <button
              onClick={() => onEdit(promptBlock)}
        ...
        <div className="flex items-center justify-between text-xs text-gray-500">
          <span>{promptBlock.usageCount} uses</span>
          <span>{new Date(promptBlock.createdAt).toLocaleDateString()}</span>
        </div>
      </div>
